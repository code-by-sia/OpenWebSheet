define(["require", "exports", '../../common/constants', './Context', "../../core/Appearance", "../../core/Sheet"], function (require, exports, constants_1, Context_1, Appearance_1, Sheet_1) {
    var CanvasRenderer = (function () {
        ...
        CanvasRenderer.prototype.renderRows = function () {
            var context = this.context;
            var height = this.height - constants_1.RowHeaderWidth - constants_1.SheetTitleHeight;
        ...
        CanvasRenderer.prototype.renderCells = function (lastColumn, lastRow) {
            ...
            for (var r = sheet.scrollRow - 1; r < lastRow; r++) {
                ...
                for (var c = sheet.scrollColumn; c <= lastColumn; c++) {
                    ...
                    var cellHeight = cell ? sheet.getCellHeight(cell) : sheet.getRowHeight(r);
                    var cellWidth = cell ? sheet.getCellWidth(cell) : sheet.getColumnWidth(c);
                    var appearance = sheet.getAppearance(c, r);
                    context.fillStyle = appearance.text;
                    this.paintCell(c, r, x, y);
                    if (y > 0 && (!cell || !cell.isMerged)) {
                        var horStyle = appearance.horizontalBorder;
                        ...
                        if (appearance.background == null || horStyle != null) {
                            context.line(x + constants_1.RowHeaderWidth, y + cellHeight, x + constants_1.RowHeaderWidth + cellWidth, y + cellHeight);
                        }
                        context.strokeStyle = (verStyle && verStyle.color) || '#eeeeee';
                        if (appearance.background == null || verStyle != null) {
                            context.line(x + constants_1.RowHeaderWidth + cellWidth, y, x + constants_1.RowHeaderWidth + cellWidth, y + cellHeight);
                        }
                    }
                    x += columnWidth;
                }
        ...
        CanvasRenderer.prototype.paintCell = function (columnId, rowId, x, y) {
            ...
            if (cell && cell.isMerged)
                return;
            var appearance = sheet.getAppearance(columnId, rowId);
            var left = x + constants_1.ColumnHeaderHeight;
            ...
            var cellWidth = cell ? sheet.getCellWidth(cell) : sheet.getColumnWidth(columnId);
            var cellHeight = cell ? sheet.getCellHeight(cell) : sheet.getRowHeight(rowId);
            ...
            if (cell && cell.label) {
                ...
                context.fillText(cell.label, x + constants_1.ColumnHeaderHeight + 5, y + 7, cellWidth);
            }
            context.unmask();
        };
