define(["require", "exports", "./Cell", "./Appearance", "../common/constants", "./formula/Evaluator"], function (require, exports, Cell_1, Appearance_1, constants_1, Evaluator_1) {
    ...
    var Sheet = (function () {
        function Sheet(title) {
            ...
            this.defaultAppearance.horizontalBorder = new Appearance_1.Border('#eeeeee');
        ...
        Sheet.prototype.getCellPos = function (name) {
        ...
        Sheet.prototype.getEvaluatedValue = function (exp) {
            if (('' + exp).indexOf(':') != -1) {
                var parts = exp.split(':');
                var a = this.getCellPos(parts[0]);
                var b = this.getCellPos(parts[1]);
                var res = [];
                for (var c = a.columnId; c <= b.columnId; c++) {
                    for (var r = a.rowId; r <= b.rowId; r++) {
                        res.push(this.getCellEvaluatedValue(c, r));
                    }
                }
                return res;
            }
            var _a = this.getCellPos(exp), rowId = _a.rowId, columnId = _a.columnId;
        ...
        Sheet.prototype.setCellValue = function (columnId, rowId, value, silent) {
        ...
        Sheet.prototype.setColumAppearance = function (columnId, Appearance) {
            this.columnAppearance[columnId] = Appearance;
            this.onChange();
        };
        Sheet.prototype.setRowAppearance = function (rowId, Appearance) {
        ...
        Sheet.prototype.getAppearance = function (columnId, rowId) {
            var appearance = new Appearance_1.Appearance();
            var cell = this.getCellAppearance(columnId, rowId);
            var col = this.getColumnAppearance(columnId);
        ...
        Sheet.prototype.style = function (app) {
            this.appearance = [];
            if (!app)
                return;
            for (var _i = 0; _i < app.length; _i++) {
                ...
                for (var _a = 0; _a < d.length; _a++) {
                    ...
                    var col = app.indexOf(d);
                    var row = d.indexOf(c);
                    this.setCellAppearance(col, row, a, true);
                }
            }
        };
        Sheet.load = function (sheet) {
            var sh = new Sheet(sheet.title);
            sh.fill(sheet.data);
